const imagedb: { [key: string]: any } = {
  acerca: [],
  murales: [
    {
      path: "./images/murales/3.jpg",
      title: "Marco Bruto y 26 de Marzo, Montevideo.",
      collection: "Montevideo",
    },
    {
      path: "./images/murales/4.jpg",
      title: "Nicaragua y General Rondeau, Montevideo.",
      collection: "Montevideo",
    },
    {
      path: "./images/murales/5.jpg",
      title: "Ellauri y Bulevar España, Montevideo.",
      collection: "Montevideo",
    },
    {
      path: "./images/murales/1.jpg",
      title: "Rambla, parada 21 de la mansa, Maldonado.",
      collection: "Maldonado",
    },
    {
      path: "./images/murales/6.jpg",
      title: "Veterinaria Barrios Jr, Maldonado.",
      collection: "Maldonado",
    },
    {
      path: "./images/murales/7.jpg",
      title: "En Estadía Pocitos, Montevideo.",
      collection: "Montevideo",
    },
    {
      path: "./images/murales/8.jpg",
      title: "Canelones y Torres García, Las Piedras.",
      collection: "Las Piedras",
    },
    {
      path: "./images/murales/9.jpg",
      title: "Torres García, Las Piedras.",
      collection: "Las Piedras",
    },
    {
      path: "./images/murales/10.jpg",
      title: "Minas y Miguelete, Montevideo.",
      collection: "Montevideo",
    },
    {
      path: "./images/murales/11.jpg",
      title: "Veterinaria Libia sucursal de Malvin, Montevideo.",
      collection: "Montevideo",
    },
    {
      path: "./images/murales/12.jpg",
      title: "25 de Mayo y Colón, Montevideo.",
      collection: "Montevideo",
    },
    {
      path: "./images/murales/2.jpg",
      title: "Venecia y Roque Saenz Peña, Ciudad de la Costa.",
      collection: "Ciudad de la Costa",
    },
  ],
  pinturas: [
    {
      path: "./images/Pinturas/acua/01.jpg",
      collection: "Acuarelas",
    },
    {
      path: "./images/Pinturas/acua/1.jpg",
      collection: "Acuarelas",
    },
    {
      path: "./images/Pinturas/acua/2.jpg",
      collection: "Acuarelas",
    },
    {
      path: "./images/Pinturas/acrilico/01.png",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/02.jpg",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/03.png",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/04.jpg",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/05.jpg",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/06.jpg",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/07.jpg",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/08.jpg",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/09.jpg",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/10.jpg",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/11.png",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/acrilico/12.png",
      collection: "Acrilícos",
    },

    {
      path: "./images/Pinturas/acrilico/13.jpg",
      collection: "Acrilícos",
    },
    {
      path: "./images/Pinturas/dripens/16.jpg",
      collection: "Dripens",
    },

    {
      path: "./images/Pinturas/dripens/17.jpg",
      collection: "Dripens",
    },
    {
      path: "./images/Pinturas/lapices/04.jpg",
      collection: "Lapices",
    },

    {
      path: "./images/Pinturas/lapices/06.jpg",
      collection: "Lapices",
    },
    {
      path: "./images/Pinturas/tinta/1.jpg",
      collection: "Tinta",
    },

    {
      path: "./images/Pinturas/tinta/2.jpg",
      collection: "Tinta",
    },
    {
      path: "./images/Pinturas/tinta/3.jpg",
      collection: "Tinta",
    },
    {
      path: "./images/Pinturas/tinta/4.jpg",
      collection: "Tinta",
    },
  ],
  fotografías: [
    {
      path: "./images/fotografia/tunel/30.jpg",
      collection: "Túnel",
    },
    {
      path: "./images/fotografia/tunel/31.jpg",
      collection: "Túnel",
    },
    {
      path: "./images/fotografia/tunel/32.jpg",
      collection: "Túnel",
    },
    {
      path: "./images/fotografia/tunel/33.jpg",
      collection: "Túnel",
    },

    {
      path: "./images/fotografia/paz/1.jpg",
      collection: "Paz",
    },
    {
      path: "./images/fotografia/paz/2.jpg",
      collection: "Paz",
    },
    {
      path: "./images/fotografia/paz/3.jpg",
      collection: "Paz",
    },
    {
      path: "./images/fotografia/paz/4.jpg",
      collection: "Paz",
    },

    {
      path: "./images/fotografia/paz/5.jpg",
      collection: "Paz",
    },
    {
      path: "./images/fotografia/paz/6.jpg",
      collection: "Paz",
    },

    {
      path: "./images/fotografia/maison/1.jpg",
      collection: "Maison",
    },
    {
      path: "./images/fotografia/maison/2.jpg",
      collection: "Maison",
    },
    {
      path: "./images/fotografia/maison/3.jpg",
      collection: "Maison",
    },
    {
      path: "./images/fotografia/maison/4.jpg",
      collection: "Maison",
    },
    {
      path: "./images/fotografia/maison/5.jpg",
      collection: "Maison",
    },
    {
      path: "./images/fotografia/maison/6.jpg",
      collection: "Maison",
    },
    {
      path: "./images/fotografia/maison/7.jpg",
      collection: "Maison",
    },
    {
      path: "./images/fotografia/maison/8.jpg",
      collection: "Maison",
    },

    {
      path: "./images/fotografia/graffiti/1.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/2.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/3.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/4.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/5.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/6.jpg",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/7.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/8.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/9.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/10.jpg",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/11.jpg",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/12.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/13.jpg",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/14.jpg",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/15.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/16.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/17.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/18.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/19.JPG",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/20.jpg",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/21.jpg",
      collection: "Graffiti",
    },
    {
      path: "./images/fotografia/graffiti/22.jpg",
      collection: "Graffiti",
    },

    {
      path: "./images/fotografia/processing/1.png",
      collection: "Altering",
    },
    {
      path: "./images/fotografia/processing/2.png",
      collection: "Altering",
    },
    {
      path: "./images/fotografia/processing/3.png",
      collection: "Altering",
    },
    {
      path: "./images/fotografia/processing/4.jpg",
      collection: "Altering",
    },
    {
      path: "./images/fotografia/processing/5.jpg",
      collection: "Altering",
    },
    {
      path: "./images/fotografia/processing/6.jpg",
      collection: "Altering",
    },
    {
      path: "./images/fotografia/processing/7.jpg",
      collection: "Altering",
    },
    {
      path: "./images/fotografia/processing/8.png",
      collection: "Altering",
    },
    {
      path: "./images/fotografia/processing/9.png",
      collection: "Altering",
    },
  ],
  animaciones: [
    {
      path: "./images/animacion/2.mp4",
      collection: "",
    },
    {
      path: "./images/animacion/02.mp4",
      collection: "",
    },
    {
      path: "./images/animacion/mujer.mp4",
      collection: "",
    },
  ],
};

export default imagedb;
